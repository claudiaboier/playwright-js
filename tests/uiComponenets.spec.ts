...
test.describe.only('Form Layouts page', () => {
    ...
    test('input fields', async ({ page }, testInfo) => {
        ...
        await expect(page.locator('#inputEmail1')).toHaveText('test2@test.com')
    })
...
test('tooltip', async ({ page }) => {
    ...
    const tooltipTopButton = page.locator('nb-card', { hasText: "Tooltip Placements" }).getByRole('button', { name: "Top" })
    await tooltipTopButton.hover()
...
test('web tables', async ({ page }) => {
    ...
    // 1 get the row by ant text in this row
